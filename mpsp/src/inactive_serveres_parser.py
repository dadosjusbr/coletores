...
# This class contains the parsers for inactive serveres:
...
def update_employee_indemnity_aug(file_name, employees):
    ...
    for row in rows:
        ...
        emp['income'].update({
            'total': round(emp['income']['total'] + alimentacao + transporte  + ferias_pc + insalubridade + grat_qualificacao, 2)
        })
        ...
        emp['income']['other'].update({
            'total': round(emp['income']['other']['total'] + insalubridade + grat_qualificacao, 2),
            'others_total': round(emp['income']['other']['others_total'] + insalubridade + grat_qualificacao, 2),
        })
...
def update_employee_indemnity_sept_to_nov(file_name, employees):
    ...
    for row in rows:
        ...
        emp = employees[matricula]
        emp['income'].update({
            'total': round(emp['income']['total'] + alimentacao + transporte + insalubridade + grat_qualificacao, 2)
        })
        emp['income']['perks'].update({
            'total': alimentacao + transporte,
            'food': alimentacao,
