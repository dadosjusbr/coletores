def parse(rows, employees):
    ...
    for row in rows:
        ...
        licence = float(row[7]) #CONVERSÃO DE LICENÇA ESPECIAL
        ...
        transportation_remu = float(row[17]) #INDENIZAÇÃO DE TRANSPORTE
        inde_license = float(row[10]) #INDENIZAÇÃO POR LICENÇA NÃO GOZADA
        found = float(row[14]) #DEVOLUÇÃO FUNDO DE RESERVA
        diff_aux =  float(row[15]) #DIFERENÇAS DE AUXÍLIOS
        gratification = float(row[16]) #GRATIFICAÇÕES EVENTUAIS
        ...
        emp['income']['other']['others'].update({
            ...
            'CONVERSÃO DE LICENÇA ESPECIAL':licence,
        ...
        emp['income']['other'].update({
            ...
            aux_edu_remu  + indemnity_vacation + licence + inde_license + found + parcelas_atraso
        ...
        emp['income']['perks'].update({
            'total': round( aux_ali + aux_ali_remu + transportation + transportation_remu + aux_saude + aux_saude_remu , 2)
        })
        emp['income'].update({
