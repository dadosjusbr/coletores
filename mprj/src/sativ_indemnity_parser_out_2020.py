def parse(rows, employees):
    ...
    for row in rows:
        ...
        aux_saude_remu = float(row[9]) #AUXÌLIO-SAUDE/OUTRAS REMUNERAÇÕES RETROATIVAS/TEMPORÁRIAS
        indemnity_vacation = float(row[9]) #INDENIZAÇÃO DE FÉRIAS NÃO USUFRUÍDAS
        licence = float(row[10]) #INDENIZAÇÃO DE LICENÇA ESPECIAL/PRÊMIO NÃO USUFRUÍDA
        transportation = float(row[7]) #AUXÍLIO-LOCOMOÇÃO/VERBAS INDENIZATÓRIAS
        transportation_remu =float(row[13]) #AUXÍLIO-LOCOMOÇÃO/OUTRAS REMUNERAÇÕES RETROATIVAS/TEMPORÁRIAS
