...
def update(remuneration, indemnsation):
    ...
    for row in indemnsation:
        ...
        if matricula in remuneration.keys():
            ...
            total_temporario = (auxilio_interiorizacao + dif_auxilio_interiorizacao + dif_auxilio_lei_8_625_93 + indenizacoes_ferias_licenca_premio + abono_pecuniario + dif_abono_pecuniario + ressarcimentos + \
                                geo + dif_geo + insalubridade + dif_insalubridade + periculosidade + dif_periculosidade + adicional_trabalho_tecnico + dif_adicional_trabalho_tecnico + \
                                grat_atividade_ensino + substituicoes + dif_substituicoes + cumulacao + dif_cumulacao + representacao_de_direcao + dif_representacao_de_direcao + grat_turma_recursal + \
                                dif_grat_turma_recursal + grat_dificil_provimento + dif_grat_dificil_provimento + grat_assessor + dif_grat_assessor + representacao_gaego + dif_representacao_gaego)
            ...
            # Peguei esse valor direto da tabela I
            # emp["income"].update(
            #     {
            ...
            # Se eu não peguei da tabela 1, jaja dou uma olhada
            # emp["income"]["other"].update(
            #     {
            #         "others_total": round(
            #             emp["income"]["other"]["others_total"] + total_temporario, 2
...
def update_fev_em_diante(remuneration, indemnsation):
    begin_row = table.get_begin_row(indemnsation, 'Matrícula')
    end_row = table.get_end_row(indemnsation, 'Total:')
    ...
    for row in indemnsation:
        ...
        if matricula in remuneration.keys():
            ...
            total_temporario = (auxilio_interiorizacao + dif_auxilio_interiorizacao + dif_auxilio_lei_8_625_93 + indenizacoes_ferias_licenca_premio + abono_pecuniario + dif_abono_pecuniario + ressarcimentos + \
                                ...
                                grat_atividade_ensino + substituicoes + dif_substituicoes + cumulacao + dif_cumulacao + representacao_de_direcao + dif_representacao_de_direcao + grat_turma_recursal + \
                                dif_grat_turma_recursal + grat_dificil_provimento + dif_grat_dificil_provimento + grat_assessor + dif_grat_assessor + representacao_gaego + dif_representacao_gaego + \
                                gratificacao_diretor_subsede + dif_gratificacao_diretor_subsede)
